...
function Student() {
  ...
  const loadStudents = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/hod/students`, {
        credentials: "include",
      });
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/hod/students`, { credentials: "include" })
      ...
      .catch(
        (err) => active && setError(err.message || "Unable to load students"),
      )
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((student) => {
      const matched =
        !term ||
        [student.userId?.name, student.userId?.email, student.rollNumber].some(
          (value) =>
            String(value || "")
              .toLowerCase()
              .includes(term),
      ...
      return (
        matched &&
        (year === "all" || String(student.year) === year) &&
        (semester === "all" || String(student.semester) === semester) &&
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-linear-to-br from-white to-cyan-50/60 p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_150px_150px_150px]">
          ...
          >
            ...
            {semesters
              .filter(
                (item) =>
                  year === "all" || Math.ceil(item / 2) === Number(year),
              )
              .map((item) => (
                <option key={item} value={item}>
                  Semester {item}
