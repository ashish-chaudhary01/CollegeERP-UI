...
function Subject() {
  ...
  const loadSubjects = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await fetch(`${API_URL}/hod/subjects`, {
        credentials: "include",
      });
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/hod/subjects`, { credentials: "include" })
      ...
      .catch(
        (err) => active && setError(err.message || "Unable to load subjects"),
      )
  ...
  const addSubject = async (event) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.message || "Unable to add subject");
      setForm(emptyForm);
      setShowAdd(false);
      await loadSubjects();
    } catch (err) {
  ...
  const saveEdit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/hod/subject/${editSubject._id}`,
        {
          method: "PUT",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
    ...
    <section className="min-h-screen">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-cyan-700"
        >
      ...
      <div className="mt-6 max-w-xl">
        <label className="relative block">
          ...
          <input
            ...
            placeholder="Search by subject name, code, year or semester"
            className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100"
          />
        </label>
      </div>
          ...
          <table className="w-full min-w-180 text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-5 py-4 font-semibold">Assigned faculty</th>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold">Add department subject</h2>
                <p className="mt-1 text-sm text-slate-500">
                  This subject is automatically linked to your department.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                disabled={saving}
                className="rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
              >
      ...
      {editSubject && (
        ...
        >
          ...
          >
            ...
            {editError && (
              <p className="mt-3 text-sm text-rose-600">{editError}</p>
            )}
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
