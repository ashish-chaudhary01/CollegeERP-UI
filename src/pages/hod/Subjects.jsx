...
function Subject() {
  ...
  // edit state
  const [editSubject, setEditSubject] = useState(null); // subject being edited
  ...
  const saveEdit = async (event) => {
    ...
    try {
      setEditSaving(true);
      setEditError("");
      const response = await fetch(`${API_URL}/hod/subject/${editSubject._id}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Unable to update subject");
      setEditSubject(null);
      await loadSubjects();
    } catch (err) {
      setEditError(err.message || "Unable to update subject");
    } finally {
      setEditSaving(false);
    }
  };
    ...
    <section className="min-h-screen">
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="overflow-x-auto">
          <table className="w-full min-w-180 text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-5 py-4 font-semibold">Subject</th><th className="px-5 py-4 font-semibold">Code</th><th className="px-5 py-4 font-semibold">Department</th><th className="px-5 py-4 font-semibold">Year</th><th className="px-5 py-4 font-semibold">Semester</th><th className="px-5 py-4 font-semibold">Assigned faculty</th><th className="px-5 py-4 font-semibold">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan="7" className="px-5 py-10 text-center text-slate-500">Loading department subjects...</td></tr> : filteredSubjects.length === 0 ? <tr><td colSpan="7" className="px-5 py-12 text-center text-slate-500"><BookOpen className="mx-auto mb-2 text-slate-300" size={26} />No subjects found in your department.</td></tr> : filteredSubjects.map((subject) => (
                <tr key={subject._id} className="transition hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 font-medium text-cyan-700">{subject.subjectCode}</td>
                  <td className="px-5 py-4 text-slate-600"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium">{subject.department}</span></td>
                  <td className="px-5 py-4 text-slate-600">{subject.year}</td>
                  <td className="px-5 py-4 text-slate-600">{subject.semester}</td>
                  <td className="px-5 py-4 text-slate-600">{subject.teacherName}</td>
                  <td className="px-5 py-4">
                    <button
                      onClick={() => openEdit(subject)}
                      className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-cyan-50 hover:text-cyan-700 transition-colors"
                    >
                      <Pencil size={13} /> Edit
                    </button>
                  </td>
      ...
      {editSubject && (
        <div
          ...
          onMouseDown={(event) => event.target === event.currentTarget && setEditSubject(null)}
        >
          <form onSubmit={saveEdit} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Update details for <span className="font-semibold text-slate-700">{editSubject.subjectName}</span>.</p>
              </div>
              <button type="button" onClick={() => setEditSubject(null)} className="rounded-lg p-1 text-slate-500 hover:bg-slate-100">
                <X size={20} />
              </button>
            </div>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-semibold text-slate-700 sm:col-span-2">
                ...
                <input
                  ...
                  onChange={(event) => setEditForm({ ...editForm, subjectName: event.target.value })}
                  className="mt-1 w-full rounded-lg border border-slate-200 p-3 font-normal outline-none focus:border-cyan-500"
                />
              </label>
              <label className="text-sm font-semibold text-slate-700">
                ...
                <input
                  ...
                  onChange={(event) => setEditForm({ ...editForm, subjectCode: event.target.value.toUpperCase() })}
                  className="mt-1 w-full rounded-lg border border-slate-200 p-3 font-normal outline-none focus:border-cyan-500"
                />
              </label>
              <label className="text-sm font-semibold text-slate-700">
              ...
              <label className="text-sm font-semibold text-slate-700 sm:col-span-2">
                ...
                <select
                  ...
                  onChange={(event) => setEditForm({ ...editForm, semester: event.target.value })}
                  className="mt-1 w-full rounded-lg border border-slate-200 bg-white p-3 font-normal outline-none focus:border-cyan-500"
                >
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setEditSubject(null)} className="rounded-lg px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100">
                Cancel
              </button>
              <button disabled={editSaving} className="rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
